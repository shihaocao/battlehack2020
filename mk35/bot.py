...
def overlord():
    ...
    for c in range(board_size):
        ...
        if False:
            ...
            if c == 0:
                # heuristic[c] = (theirs[c] + theirs[c+1])*1.5 - 2*deepest[c]*2 + ourplayers[c]
                heuristic[c] = (theirs[c] + theirs[c+1])*1.5 - .5*deepest[c]*deepest[c] + ourplayers[c]

            elif c == board_size - 1:
                heuristic[c] = (theirs[c] + theirs[c-1])*1.5 - .5*deepest[c]*deepest[c] + ourplayers[c]
            else:
                heuristic[c] = theirs[c] + theirs[c-1] + theirs[c+1] - .5*deepest[c]*deepest[c] + ourplayers[c]
...
def flank_secured():
    ...
    # if check_space_wrapper(row -2*forward, col, board_size) == team:
    #     balls += 1
    #     bh += 1
    #     flank += 1
    ...
    # attack_thresh = .2*(10 + 2*abs(row - goal)-abs(row-goal)*abs(row-goal)/8)
    attack_thresh = 0
    if abs(row - index) < 3:
        tt = 1
    else:
    ...
    if ene == 0:
        return 3
    if ene == 1 and col != 0 and col != board_size-1 and flank > tt/2 and sho > 0:
        attack_timer += 1
        if attack_timer > attack_thresh:
            return 3
    if ene == 1 and (col == 0 or col == board_size-1) and flank > tt/2 and sho > 0:
        # dlog("REEEEEEEEEEE")
        attack_timer += 1
        if attack_timer > attack_thresh:
            return 3
    # dlog(str(attack_timer))
    if flank > tt and sho > 0:
        attack_timer += 1
        tc = too_close() 
        if local_timer > 40:
            return 3
