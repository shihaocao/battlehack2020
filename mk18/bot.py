...
def overlord():
    global team, opp_team, robottype, forward, row, col, pawn_state, index
    ...
    if(timer) < 500:
        ...
        dlog("OTHER WAY")
        pref = [mult if (c < half) else -mult for c in range(board_size)]
    ...
    for c in range(board_size):
        ...
        if False:
            ...
            heuristic[c] = ourplayers[c] - theirs[c] + pref[c]
...
def flank_secured():
    global team, opp_team, robottype, forward, row, col, pawn_state
    global local_timer, index, attack_timer, attack_thresh
    ...
    if ene == 0:
        return 3
    elif ene == 1 and col != 0 and col != board_size and flank > 4 and sho > 1 and attack_timer > attack_thresh:
        attack_timer += 1
        return 3
    elif flank > tt and attack_thresh > 5:
        attack_timer += 1
        if too_close() and local_timer > 40:
            return 3
        if too_close():
            local_timer += 1
            return -2
